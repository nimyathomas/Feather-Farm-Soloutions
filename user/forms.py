...
class StakeholderUserForm(forms.ModelForm):
    ...
    def clean_coopcapacity(self):
        coopcapacity = self.cleaned_data.get('coopcapacity')
        if coopcapacity is not None and coopcapacity <= 0:
            raise forms.ValidationError("Coop capacity must be a positive number greater than zero.")
        return coopcapacity
    ...
    # Validate file size (ensure it's under 5 MB)
        ...
        if pdf.size > max_size_in_mb * 1024 * 1024:  # Convert MB to bytes
            raise ValidationError(f"The PDF file is too large. The file size must not exceed {max_size_in_mb} MB.")
